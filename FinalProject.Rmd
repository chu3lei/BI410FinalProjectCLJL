...
#ecoregions_shp <- st_read("../ecoregion.shp") # Chu 
...
<<<<<<< HEAD
# excel file of native or nonnative plants
install.packages("writexl")
library(writexl)
write_xlsx(data.frame(Species = plant_sp_list, "../Desktop/Bi410L")

install.packages("readxl")
library(readxl)
df <- read_excel("../BI410FinalProjectCLJL/plant_species_list.xlsx", sheet = "Sheet1") 

names(df)
names(coastal_oba)

names(df)[names(df) == "Species"] <- "speciesPlant"

merged_df <- coastal_oba %>%
  left_join(df, by = "speciesPlant") 
...
obs_stat <- obs$n_bee_species[obs$IsNative == TRUE] -
            obs$n_bee_species[obs$IsNative == FALSE]
...
#I think this only needed to be done once to extract the data
#install.packages("writexl")
#library(writexl)
#write_xlsx(data.frame(Species = plant_sp_list, "../Desktop/Bi410L")
>>>>>>> 3a680a921e6aea65bfc18e2a30592ef5cae80bdc
```
...
#Note: Rubus armeniacus and Rubus bifrons are synonyms. 
...
#Note: Solidago elongata and Solidago canadensis are synonyms. 
